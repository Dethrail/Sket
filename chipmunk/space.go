...
func (space *Space) Step(dt vect.Float) {
	...
	//log.Printf("Shapes = %d", space.activeShapes.Count())
	...
	biasCoef := vect.Float(0.2) //vect.Float(1.0 - math.Pow(float64(space.collisionBias), float64(dt)))
	invdt := vect.Float(1 / dt)
	for _, arb := range space.Arbiters {
		arb.preStep(invdt, space.collisionSlop, biasCoef)
	}
	...
	//dt_coef := vect.Float(1)
	//if prev_dt != 0 {
	//	dt_coef = dt / prev_dt
	//}

	for _, arb := range space.Arbiters {
		arb.applyCachedImpulse(vect.Float(1))
	}

	for _, con := range space.Constraints {
		con.ApplyCachedImpulse(vect.Float(1))
	}
...
func SpaceCollideShapes(a, b *Shape, space *Space) {
	...
	if preSolveResult &&
		...
		!sensor && arb.state != arbiterStateIgnore {
		space.Arbiters = append(space.Arbiters, arb)
	} else {
